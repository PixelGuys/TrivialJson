...
public abstract class JsonParser {
	...
	private static ThreadLocal<String> filePathForErrorHandling = new ThreadLocal<String>();
	...
	public static JsonObject parseObjectFromString(String text) {
		filePathForErrorHandling.set("text");
		return _parseObjectFromString(text);
	}
	...
	public static JsonObject parseObjectFromFile(String path){
		String text = "";
		filePathForErrorHandling.set(path);
		try {
			text = new String(Files.readAllBytes(Paths.get(path)), Json.encoding);
		} catch (IOException e) {
			Json.handleException(e);
			return new JsonObject();
		}
		return _parseObjectFromString(text);
	}

	public static JsonElement parseFromString(String text) {
		filePathForErrorHandling.set("text");
		char[] characters = text.trim().toCharArray(); // Remove leading and trailing spaces and convert to char array.
		int[] index = new int[] {0};
		return parseValue(characters, index);
	}
	...
	public static void storeToFile(JsonElement json, String path) {
		...
		try {
			Files.write(file.toPath(), text.getBytes(Json.encoding));
		} catch (IOException e) {
			Json.handleException(e);
		}
	}

	private static JsonObject _parseObjectFromString(String text) {
		char[] characters = text.trim().toCharArray(); // Remove leading and trailing spaces and convert to char array.

		if (characters[0] != '{') {
			Json.errorHandler.accept("Expected the json data to start with an object.");
			return new JsonObject();
		}
		int[] index = new int[] {1};
	...
	private static void printError(int index, char[] chars, String message) {
		...
		String filePath = filePathForErrorHandling.get();
		Json.errorHandler.accept("Syntax error reading json from " + filePath + " in line " + line + ": " + message);
	}
	...
	private static JsonElement parseValue(char[] chars, int[] index) {
		...
		switch(chars[index[0]]) {
			...
			case '-':
				...
				while (end < chars.length && !Character.isWhitespace(chars[end]) && chars[end] != ',' && chars[end] != '}' && chars[end] != ']') {
					switch(chars[end]) {
					case '0':
					case '1':
					case '2':
